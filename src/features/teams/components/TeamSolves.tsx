...
import { CheckCircle2, ListChecks, Trophy, X } from 'lucide-react'

import EmptyState from '@/shared/components/EmptyState'
import { Button, Card, CardContent, Dialog, DialogContent, DialogTitle } from '@/shared/ui'
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between px-1">
        <h2 className={cn('text-xs font-bold uppercase tracking-widest text-muted-foreground', "flex items-center gap-2 !text-gray-900 dark:!text-white")}>
      ...
      <Dialog open={showAllSolves} onOpenChange={setShowAllSolves}>
        <DialogContent className={'w-[calc(100vw-32px)] sm:w-full fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 max-w-3xl bg-card border border-border text-card-foreground rounded-2xl p-0 overflow-hidden' + " fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 !rounded-2xl border-none p-0 overflow-hidden"}>
          <div className="sticky top-0 z-10 flex items-center justify-between border-b border-border bg-background/90 px-5 py-4 backdrop-blur-xl">
