...
const ChallengeDetailDialog: React.FC<ChallengeDetailDialogProps> = ({
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => { if (!isOpen) onClose() }}>
      <DialogContent
        className="w-[calc(100vw-32px)] sm:w-full fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 max-w-2xl bg-white/70 dark:bg-[#0a0d14]/80 border border-gray-200 dark:border-gray-800 text-gray-900 dark:text-gray-100 rounded-2xl shadow-2xl backdrop-blur-xl p-0 overflow-hidden [&_button.absolute.right-4.top-4]:block md:[&_button.absolute.right-4.top-4]:hidden [&_button.absolute.right-4.top-4]:text-gray-500 dark:[&_button.absolute.right-4.top-4]:text-gray-400 w-[95vw] h-[90vh] flex flex-col"
        ...
        <div className="p-4 md:px-6 pb-0 shrink-0">
          <div className="flex flex-col gap-3 mb-5">
            ...
            <div className="flex items-start justify-between gap-4">
              <DialogTitle><h2 className="select-text text-2xl font-black text-gray-900 dark:text-gray-100 tracking-tight leading-tight">
                  {challenge.title}
                </h2></DialogTitle>
              {eventName && (
