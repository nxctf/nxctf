  ...
  return (
    ...
    >
      {visibleCategories.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {visibleCategories.map(({ category, total_challenges, solvedCount, progress }) => (
            <div
              key={category}
              className={cn("p-4", 'bg-card border border-border rounded-xl')}
            >
      ...
      {activeDifficulties.length > 0 && (
        <div className={cn("p-4", 'bg-card border border-border rounded-xl')}>
          ...
          <div className="flex h-3 overflow-hidden rounded-full bg-gray-200/50 dark:bg-gray-800/50 gap-0.5">
            {activeDifficulties.map(({ difficulty, total_challenges, solvedCount, bgColor }) => {
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {activeDifficulties.map(({ difficulty, total_challenges, solvedCount, bgColor, textColor, borderColor }) => (
              <span
                key={difficulty}
                className={cn("inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 font-medium transition-colors", borderColor, textColor, 'text-xs font-medium text-muted-foreground')}
              >
