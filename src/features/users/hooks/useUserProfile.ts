...
export function useUserProfile(userId: string | null, isCurrentUser: boolean) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const profile = await UserProfileService.getUserProfileLite(userId)
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      ...
      try {
        const detail = await UserProfileService.getUserDetail(
          userId,
        ...
        if (detail) {
          const firstBlood = await ChallengeService.getFirstBloodChallengeIds(detail.id)
          const solvedIds = new Set((detail.solved_challenges || []).map(c => c.id))
          setFirstBloodIds(firstBlood.filter(id => solvedIds.has(id)))

          const totals = await UserStatsService.getCategoryTotals(
            effectiveSelectedEventId,
            effectiveSelectedEventMode
          )
          setCategoryTotals(totals)

          const diffTotals = await UserStatsService.getDifficultyTotals(
            effectiveSelectedEventId,
            effectiveSelectedEventMode
          )
          ...
          if (APP.teams.enabled) {
            const { team, members } = await TeamService.getTeamByUserId(detail.id)
  ...
  const handleShowUnsolved = async () => {
    ...
    try {
      ...
      const allChallenges = await ChallengeService.getChallenges(userId || undefined, false, challengeEventParam)
